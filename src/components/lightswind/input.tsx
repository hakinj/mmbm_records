  ...
  ({ className, type, ...props }, ref) => {
    ...
    return (
      <motion.input
        type={type}
        className={cn(
          `flex h-10 w-full rounded-md border border-gray-200 dark:border-gray-800 bg-background px-3 py-2 text-base ring-offset-background 
        ...
        animate={{
          ...
          boxShadow: isFocused ? "0 0 0 2px var(--ring), 0 0 0 4px var(--ring-offset-background)" : "none", // Add a custom shadow or combine with Tailwind ring
          borderColor: isFocused ? "var(--ring)" : "var(--border) ", // Animate border color (if needed, Tailwind ring handles this well)
        }}
        transition={{
          type: "spring",
          stiffness: 400,
